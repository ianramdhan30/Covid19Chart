...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tNegara : String? = intent.getStringExtra(EXTRA_COUNTRY)
        val tTanggal : String? = intent.getStringExtra(EXTRA_LATESTUPDATE)
        val tHasilNewConfirmed : String? = intent.getStringExtra(EXTRA_NEWCONFIRMED)
        val tHasilNewDeath : String? = intent.getStringExtra(EXTRA_NEWDEATH)
        val tHasilNewRecovered : String? = intent.getStringExtra(EXTRA_NEWRECOVERED)
        val tHasilTotalConfirmed : String? = intent.getStringExtra(EXTRA_TOTALCONFIRMED)
        val tHasilTotalDeath : String? = intent.getStringExtra(EXTRA_TOTALDEATH)
        val tHasilTotalRecovered : String? = intent.getStringExtra(EXTRA_TOTALRECOVERED)
        val idCountry: String? = intent.getStringExtra(EXTRA_COUNTRYID)
        val editor: SharedPreferences.Editor = sharedPreferences.edit()

        txt_name_country.text = tNegara.toString()
        txt_latest_update.text = tTanggal.toString()
        txt_hasil_new_confirmed_currently.text = formatter.format(tHasilNewConfirmed.toString().toDouble())
        txt_hasil_new_death_currently.text = formatter.format(tHasilNewDeath.toString().toDouble())
        txt_hasil_new_recovery_currently.text = formatter.format(tHasilNewRecovered.toString().toDouble())
        txt_hasil_total_confirmed_currently.text = formatter.format(tHasilTotalConfirmed.toString().toDouble())
        txt_hasil_total_death_currently.text = formatter.format(tHasilTotalDeath.toString().toDouble())
        txt_hasil_total_recovery_currently.text = formatter.format(tHasilTotalRecovered.toString().toDouble())
        editor.putString(tNegara, tNegara)
        editor.apply()
        editor.commit()

        val simpanNegara = sharedPreferences.getString(tNegara, tNegara)
                ...
                if (response.isSuccessful) {
                    ...
                    while (i < getListDataCorona.size) {
                        for (s in getListDataCorona) {
                            ...
                            val barEntry2 = BarEntry(i.toFloat(), s.Death.toFloat())
